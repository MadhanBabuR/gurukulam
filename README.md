...
| Contact section address + social links | index.html Contact section | spec called for a full address and social links — currently the Contact section has neither (the redundant address/link block was removed to avoid duplicating the footer's); add a real street address (if to be published) and social media links once available |
